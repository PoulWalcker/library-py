...
from app.crud.librarian import (
    create_librarian,
    get_librarian_by_email,
    get_librarian_by_id,
    Librarian,
)
...
def register_librarian(new_librarian: LibrarianCreate, db: Session = Depends(get_db)):
    existing_librarian = (
        db.query(Librarian).filter_by(email=new_librarian.email).first()
    )
    if existing_librarian:
        raise HTTPException(
            status_code=400, detail='Librarian with this email already registered.'
        )
...
def login(
    ...
    if not librarian or not verify_password(
        librarian_password, librarian.hashed_password
    ):
        raise HTTPException(
            status_code=401,
    ...
    access_token = create_access_token(
        data={"sub": str(librarian.id)}, expires_delta=access_token_expires
    )
